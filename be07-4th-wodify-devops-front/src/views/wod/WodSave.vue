<template>
  <v-container style="min-width: 345px; min-height: 600px; max-width: 800px">
      ...
      <v-dialog v-model="timePickerVisible" persistent width="400px">
        <v-card class="dark-card">
          ...
          <v-card-actions class="time-picker-actions">
            ...
            <v-btn text @click="timePickerVisible = false" class="bold-button"
              >Cancel</v-btn
            >
          </v-card-actions>
        </v-card>
      ...
      <v-dialog v-model="roundsPickerVisible" persistent width="180px">
        <v-card class="dark-card">
          ...
          <v-card-actions class="time-picker-actions">
            ...
            <v-btn text @click="roundsPickerVisible = false" class="bold-button"
              >Cancel</v-btn
            >
          </v-card-actions>
        </v-card>
    ...
    <!-- 동적 입력 필드 -->
    <div
      v-for="(wodDetSaveReqDto, index) in wodSaveReqDto.wodDetSaveReqDtoList"
      :key="index"
    ...
    >
      ...
      <v-text-field
        ...
        placeholder="Enter exercise contents"
      ></v-text-field>
    </div>

    <!-- 버튼들 -->
    <div style="display: flex; justify-content: flex-end">
      <v-btn @click="addRandomWodDet" color="black" style="margin-right: 2px"
        >랜덤+</v-btn
      >
    ...
    <v-dialog v-model="showNoDataModal" persistent max-width="400px">
      <v-card>
        ...
        <v-card-actions>
          <v-btn class="custom-btn" @click="closeNoDataModal">확인</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="showDataModal" persistent max-width="400px">
      <v-card>
        ...
        <v-spacer></v-spacer>
        <v-card-actions>
          <v-btn class="custom-btn" @click="addWodData">추가</v-btn>
          <v-btn class="custom-btn" @click="closeDataModal">취소</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="showSubmitModal" persistent max-width="400px">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn class="custom-btn" @click="submitForm">확인</v-btn>
          <v-btn class="custom-btn" @click="closeSubmitModal">취소</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="showResultModal" persistent max-width="400px">
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn class="custom-btn" @click="confirmResultModal">확인</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-container>
    ...
    async addRandomWodDet() {
      try {
        ...
        const countResponse = await axios.get(
          `${process.env.VUE_APP_API_BASE_URL}/wod/count`
        );
        ...
        // 3. 해당 id의 데이터 가져오기
        const randomDataResponse = await axios.get(
          `${process.env.VUE_APP_API_BASE_URL}/wod/random/${randomIndex}`
        );
    ...
    async submitForm() {
      try {
        ...
        const response = await axios.post(
          `${process.env.VUE_APP_API_BASE_URL}/wod/save`,
          this.wodSaveReqDto
        );
