    ...
    <div class="container">
      <v-container>
        <div>
          ...
          <v-row>
            <v-col>
              <v-form @submit.prevent="searchBoxes">
                <v-row>
                  <v-col cols="auto">
                    <v-select v-model="searchType" :items="searchOptions" item-title="text" item-value="value" label="Search By">
                    </v-select>
                  </v-col>
                  <v-col>
                  ...
                  <v-table style="table-layout: fixed; width: 100%;">
                    ...
                    <tbody>
                      <tr v-for="(box, index) in boxes" :key="`box-${index}`">
                        <td>
                          <v-img :src="box.logoPath" style="height: 50px; width: 50px; border-radius: 50%;"></v-img>
                        </td>
                        <td>{{ box.name }}</td>
                        <td>{{ box.address }}</td>
          ...
          <v-pagination v-model="currentPage" :length="totalPages" :total-visible="5" class="mt-4" @input="fetchBoxes" />
        </div>
      </v-container>
    </div>
  </template>
