<template>
    <v-container>
        <v-row v-if="post" align="center">
            ...
            <v-col cols="auto" v-if="isAuthor"> <v-btn color="warning" @click="goToEditPost">수정</v-btn> </v-col>
            <v-col cols="auto" v-if="isAuthor"> <v-btn color="error" @click="deletePost">삭제</v-btn> </v-col>
            <v-col cols="auto">
        ...
        <div v-if="post">
            ...
            <h2>댓글</h2>
            <v-list>
                <comment-item v-for="comment in post.comments" :key="comment.id"
                    :comment="comment" :postId="post.id" :currentUserId="this.currentUserId"
                    @comment-deleted="removeComment"></comment-item>
            </v-list>
        </div>
    </v-container>
</template>
